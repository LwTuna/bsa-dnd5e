export class Warhammer implements SystemApi {
    ...
    get configSkills(): SkillConfig[] {
        return Object.entries(game["wfrp4e"].config.skills)
            .map(skills => {
                // @ts-ignore
    ...
    get configAbilities(): AbilityConfig[] {
        return Object.entries(game["wfrp4e"].config.abilities).map(([id,value]) => {
            // @ts-ignore
    ...
    get configCurrencies(): CurrencyConfig[] {
        ...
        const currencies = CONFIG["WFRP4E"].currencies as {[key:string]:Wfrp4eCurrency}
